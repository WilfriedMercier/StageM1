...
def PROFCL_Fit(RA, Dec, prob_membership,
    ...
    # check model
    if model != "NFW" and model != "coredNFW" and model != "NFWtrunc" and  model != "Uniform":
...
if __name__ == '__main__':
    ...
    models              = ("NFW", "coredNFW", "NFWtrunc", "Uniform")
...
#                          "TNC" : "tnc", "Diff-Evol" : "de", "test" : "t"}
    fmt                 = "{:19} {:7} {:40} {:11} {:.1e} {:d} {:d} {:d} {:5} {:8} {:8.4f} {:8.4f} {:7.3f} {:5.3f} {:3.0f} {:6.2f} {:6.3f} {:6.1f} {:9.2f} {:9.2f} {:5d} {:8.4f} {:8.4f} {:7.3f} {:5.3f} {:3.0f} {:6.2f} {:8.3f}"
    fmt2                = fmt + "\n"
    ...
    if test_flag == 'd':
        ...
        if auto == "manual":
            if "data_galaxies_dir" in locals():
                ...
                data_galaxies_dir = input("Enter galaxy data directory: ")
        data_galaxies_file = input("Enter galaxy file: ")
        galaxy_file = data_galaxies_dir + '/' + data_galaxies_file
        RA_Dec_cluster = input("Enter RA and Dec of cluster: ")
        RA_cen_init0, Dec_cen_init0 = map(float, RA_Dec_cluster.split())
        id_cluster_min = 1
        id_cluster_max = 1
        model_test_tmp = input("Enter model: ")
        column_z = 0
    ...
    # extract parameters from data file name
    galaxy_file = "mock_NFW_e2_PA30_a0.5_N100_Sbg5.0_RA20.0_Dec10.0.dat"
    if test_flag == 'm' and 2 == 1:
    ...
    if test_flag == "d":
        new_detection_file = "AMICO_detection.fits"
        ...
        new_halo_association_file = "AMICO_hassociation.fits"
        ...
        new_cluster_association_file = "AMICO_cassociation.fits"
        with fits.open(new_cluster_association_file) as hdu:
            data                                                    = hdu[1].data
            ID_g, ID_c, p_c, RA, Dec                                = data['ID_g'], data['ID_c'], data['p_c'], data['RA'], data['Dec']

    extended_data_flag = False
    if extended_data_flag:
        extended_data_file  = open("NFW_TNC_0.0001.output", "w")
    ...
    f_outout = open("medsep_amas.dat", "a")
    ...
    for id_cluster in cluster_range:
        ...
        if test_flag in ('a', 'A'):
            galaxy_file = data_galaxies_dir + '/' + galaxy_file0
        if test_flag == 'm':
            galaxy_file0 = galaxy_file

        if verbosity >= 1:
            print("galaxy_file = ", galaxy_file)

        # cluster center
        galaxy_file = "Mocks_Mamon/MockNFW1280ellip0.0loga-2.08PA110center0.0_0.0With_background1num" + str(id_cluster) + ".dat"
        ...
        if test_flag in ("A", "a", "f"):
            ...
            RA_gal, Dec_gal = np.genfromtxt(galaxy_file, usecols=(1,2), unpack=True, skip_header=1)
        elif test_flag == "d":
            RA_gal          = RA[ID_c == id_cluster]
            Dec_gal         = Dec[ID_c == id_cluster]
            ...
            #RA_gal  = RA_H
            #Dec_gal = Dec_H
        #prob_membership = 0*RA_gal+1

        N_data = len(RA_gal)
        print("Ndata:", N_data)

        if test_flag == "d":
            file_tmp = open(galaxy_file, "w")
            for i in range(N_data):
                file_tmp.write(str(RA_gal[i]) + " " + str(Dec_gal[i]) + "\n")
            print(galaxy_file)
            file_tmp.close()
            RA_gal, Dec_gal = np.genfromtxt(galaxy_file, usecols=(0,1), unpack=True)
        ...
        if median_flag in ('y', 'o'):
            ...
            f_outout.write(str(log_median_separation) + " " + str(id_cluster) + " " + str(SNR_clust) + " " + str(mass_AMICO) + "\n")
            continue
            if verbosity >= 1:
        ...
        if test_flag == "d" and 1 == 2:
            distance    = proper_motion_distance(z_clust)
            size        = np.max(R_sky)*distance
            ...
            for ellipticity_flag in ellipticity_flags:
                ...
                for background_flag in background_flags:
                    ...
                    for model in models:
                        ...
                        if model == "NFWtrunc":
                            # cutoff radius
                            R_cut_minallow = 1.0 * R_max
                            R_cut_maxallow = 3.0 * R_max
                            R_cut          = 1.5 * R_max
                        else:
                        ...
                        if median_flag != 'o':
                            ...
                            if method in ("Nelder-Mead","BFGS","Powell") and not recenter_flag:
                                ...
                                DeltaDec = 0.
                            denom = \
                                    ProjectedNumber_tilde(R_max/a_bestfit,model,ellipticity_bestfit,DeltaCenter_bestfit,R_cut_bestfit) \
                                    - \
                                    ProjectedNumber_tilde(R_min/a_bestfit,model,ellipticity_bestfit,DeltaCenter_bestfit,R_cut_bestfit)
                            Nofa_bestfit = num/denom

                        # print results to file
                        if extended_data_flag and test_flag == "d":
                            ...
                            print(str(id_cluster) + " " + method + " " + model + " " + str(round(tolerance,5)) + " " + str(round(log_scale_radius_pred,3)) + " " + 
                                                     ...
                                                     str(round(fit_results.fun,2)) + " " + str(round(mass_AMICO,3)) + " " + 
                                                     str(round(SNR_clust,3)) + " " + str(round(size,2)) + " " + str(round(time_taken,3))  + "\n")
                        elif median_flag not in ('ay', 'o'):
                            #print(log_scale_radius_bestfit, log_scale_radius_pred)
                            print(str(log_scale_radius_bestfit) + " " + str(log_scale_radius_pred) + " " + str(R_cut_bestfit) + " " + str(id_cluster) + " " + str(SNR_clust) + " " + str(mass_AMICO) + "\n")
                            #f_outout.write(str(log_scale_radius_bestfit) + " " + str(log_scale_radius_pred) + "\n")
                            f_outout.write(str(log_scale_radius_bestfit) + " " + str(log_scale_radius_pred) + " " + str(R_cut_bestfit) + " " + str(id_cluster) + " " + str(SNR_clust) + " " + str(mass_AMICO) + "\n")

        # print also median separation using same format
        if median_flag in ('ay', 'o'):
            print("MEDIANNNNNNNNNNNNNNNNNNN")
