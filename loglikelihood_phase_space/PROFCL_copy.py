...
* Version 1.15 (15 May 2018) by W. Mercier
...
def PROFCL_prob_uv(u, v, R_min, R_max, scale_radius, elliipticity, background, model):
    ...
    #numerator   = 2*np.pi*(R_circ/a)*(Nofa / (np.pi*a*a*bova) * SurfaceDensity_tilde(R_circ/a,model) + background)
...
if __name__ == '__main__':
    ...
    cluster_richness, cluster_ellipticity, cluster_PA, background, fits_flag               = 1280, 0.4, 10, 3000, True
        ...
        try:
            ...
            with f_test:
                ...
                [verbosity, test_flag, model_test_tmp, cluster_richness, cluster_ellipticity, cluster_PA, background, cluster_loga, cluster_cen_RA, cluster_cen_Dec, Rmaxovervir, do_center, do_ellipticity,
                 N_points, do_background, method, tolerance, id_cluster_min_max, min_members, cluster_list, data_galaxies_dir, median_flag, output_file, detection_file, association_file] = params_list
    ...
    my_list = [verbosity, test_flag, model_test_tmp, cluster_richness, cluster_ellipticity, cluster_PA, background, cluster_loga, cluster_cen_RA, cluster_cen_Dec, Rmaxovervir, do_center, do_ellipticity,
            ...
            for ellipticity_flag in ellipticity_flags:
                ...
                for background_flag in background_flags:
                    ...
                    for model in models:
                        ...
                        #print(log_background)
                        #plt.plot(RA, Dec, "r,")
                        #plt.plot(leftmost_RA, leftmost_Dec, "bo", rightmost_RA, rightmost_Dec, "yo", topmost_RA, topmost_Dec, "go", bottommost_RA, bottommost_Dec, "o")
                        #plt.show()
                        ...
                        if method == 'test':
                            ...
                            while cont == 'y':
                                ...
                                #log_scale_radius = float(input("Enter log_scale_radius (deg): "))
                                ...
                                #background = float(input("Enter log10(background) (deg^{-2}): "))
                                #log_background = np.log10 (background * 3600.)        # in deg^{-2}
                                # set min and max allowed values to desired values
                                log_scale_radius, RA_cen, Dec_cen, ellipticity, PA = np.genfromtxt("input_phase_space.dat", usecols=(0, 1, 2, 3, 4), unpack=True)
                                log_background = -99
                                ...
                                cont = "n"
                                f = open("output_phase_space_RA" + str(RA_cen) + "_Dec" + str(Dec_cen) + ".dat", "a")
                                f.write(str(log_scale_radius) + " " + str(ellipticity) + " " + str(PA) + " " + str(lnlikminus) + "\n")
                                f.close()
                        ...
                        if median_flag != "o":
                            ...
                            print (" date/time          cluID/file              method      tol c e b int model    RA0       Dec0    log_r_{-2} ellip  PA log_bg    Nr_minus   -lnL       BIC      Npasses     RA_pred       Dec_pred        log_r_pred      ellip_pred       PA_pred       log_bg_pred       Computation_time(s)")
