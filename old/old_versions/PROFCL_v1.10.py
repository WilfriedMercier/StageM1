...
def Random_radius(Xmax,model,Npoints,Nknots):
    """Random R/r_{-2} for circular NFW model
    ...
    asinhX_spline = interpolate.splev(np.arcsinh(q), spline, der=0, ext=2)
    return (np.sinh(asinhX_spline))

def Random_xy(Rmax,model,Npoints,Nknots,ellipticity,PA):
    """Random x & y (in deg) for circular model (NFW or cNFW)
    ...
    PA_random = np.pi * np.random.random_sample(Npoints) # in rd
    u_random = R_random * np.cos(PA_random)
    v_random = R_random * np.cos(PA_random) / (1-ellipticity)
...
def ProjectedNumber_tilde_ellip_NFW(X,ellipticity):
    ...
    if N_points == 0:
        ...
        # draw 10^7 points in circle of radius R_region/(b/a)

        # print("ProjectedNumber_tilde_ellip_NFW: log_scale_radius = ", log_scale_radius)
        # print("ProjectedNumber_tilde_ellip_NFW: scale_radius = ", scale_radius)
        (x_rand,y_rand) = Random_xy(X*10.**log_scale_radius/(1-ellipticity),model,N_points,N_knots,ellipticity,PA)     #Commentaire -> PA, model et log_scale_radius pas definis dans cette fonction
        ...
        Mproj = len(R_rand_sq_in_scale_radius) / len(R_rand_sq_in_circle)
...
def ProjectedNumber_tilde_Uniform(X):  #Commentaire -> Il y a deja une fonction nommee pareil plus haut (quasiment identique)
...
def ProjectedNumber_tilde(X,model):
    ...
    if verbosity >= 4:
        print("ProjectedNumber_tilde: ellipticity=",ellipticity)                             #Commentaire -> ellipticity pas definie ici
    if X < -TINY:
        ...
        return ProjectedNumberUniform(X)                 #Commentaire -> Pas de fonction s'appelant ainsi (peut etre ProjectedNumber_tilde_Uniform ?)
    else:
...
def PROFCL_prob_uv(u, v, R_min, R_max, scale_radius, elliipticity, background, model):
    ...
    if Nofa < 0:
        if background_flag:
            ...
            raise print("Nofa = ", Nofa, " N_tot = ", N_tot, " background=", background, " dR2 = ", dR2, " R_min = ", 
                      R_min, " R_max = ", R_max, " dNproj_tilde =", dNproj_tilde, " N_p(R_max) = ", Nproj_tilde_R_max, " N_p(R_min) = ", Nproj_tilde_R_min)
    # numerator = (2. * R_circ * 
    #                (
    ...
    if verbosity >= 2:
        print("bova=",bova)
    R_circ = np.sqrt(u*u + (v/bova)*(v/bova))                                                                              #Commentaire -> deja calcule dans la fonction precedente (peut le passer en argument)
    # numerator = 2*np.pi * R_circ * (Nofa / (a*a) / (np.pi * bova) * SurfaceDensity_tilde(R_circ/a,model) + background)
    numerator = Nofa / (a*a) / (np.pi * bova) * SurfaceDensity_tilde(R_circ/a,model) + background                          #Commentaire -> idem X_circ=R_circ/a deja calcule auparavant
...
def PROFCL_LogLikelihood(params, RA, Dec, prob_membership, R_min, R_max, model):
    ...
    # check that cluster center position are floats or integers
    CheckTypeIntorFloat(RA_cen,'PROFCL_LogLIkelihood','RA_cen')
    CheckTypeIntorFloat(Dec_cen,'PROFCL_LogLIkelihood','Dec_cen')
    
    # check that ellipticity and position angle are floats or integers
    ...
    if R_min <= 0. and model == 'NFW':
        ...
        raise print('ERROR in PROFCL_lnlikelihood: R_min must be >= 0 for coredNFW model')              #Commentaire -> Quid de l'uniforme ?
...
if __name__ == '__main__':
    ...
    fmt  = "{:19} {:5} {:40} {:12} {:.1e} {:d} {:d} {:d} {:5} {:8} {:8.4f} {:8.4f} {:10.3f} {:5.3f} {:3.0f} {:6.2f} {:6.1f} {:15.7G} {:15.7G} {:5d}"
    ...
    try:
        ...
        with f_test:
            ...
            [verbosity,test_flag,model_test_tmp,cluster_richness,cluster_ellipticity,cluster_PA,background,Rmaxoverrvir,do_center,do_ellipticity,N_points,do_background,method,tolerance,id_cluster_min_max,min_members,data_galaxies_dir] = params_list

    # verbosity

    if 'verbosity' in locals():
        verbosity = int(input("Enter verbosity (0 -> minimal, 1 -> some, 2 -> more, 3 -> all) [%s]: "%verbosity) or verbosity)
        # verbosity = int(input("Enter verbosity (0 -> minimal, 1 -> some, 2 -> more, 3 -> all) [%s]: "%verbosity + chr(8)*4))
    else:
        verbosity = int(input("Enter verbosity (0 -> minimal, 1 -> some, 2 -> more, 3 -> all): "))
    ...
    if do_ellipticity == 'e' and do_center == 'n':
        if 'N_points' in locals():
            N_points = input("Enter 0 for approximate analytical or N_points for Monte-Carlo [%s]: "%N_points) or N_points
        else:
            N_points = input("Enter 0 for approximate analytical or N_points for Monte-Carlo: ")
        N_points = int(N_points)
        if N_points > 0 and N_points < 10000:
    ...
    my_list = [verbosity,test_flag,model_test_tmp,cluster_richness,cluster_ellipticity,cluster_PA,background,Rmaxoverrvir,do_center,do_ellipticity,N_points,do_background,method,tolerance,id_cluster_min_max,min_members,data_galaxies_dir]
    ...
    for id_cluster in range(id_cluster_min,id_cluster_max+1):
        ...
        if test_flag == "d" or test_flag == "f":
            ...
            if float(background) > 0.:
                galaxy_file0 = 'Acad_' + model_test + cluster_richness + '_c3_skyRA15Dec10_ellip' + cluster_ellipticity_10 + 'PA' + cluster_PA + '_bg' + background + '_Rmax' + Rmaxoverrvir + '.dat'
            else:
                galaxy_file0 = 'Acad_' + model_test + cluster_richness + '_c3_skyRA15Dec10_ellip' + cluster_ellipticity_10 + 'PA' + cluster_PA + '.dat'
        elif test_flag == "A":
            galaxy_file0 = 'MockNFW' + cluster_richness + 'ellip' + cluster_ellipticity_10 + 'loga2.08PA50center0With_background20num' +  str(id_cluster) + '.dat'

        if test_flag == "a" or test_flag == "A":
            galaxy_file = 'MOCKS/' + galaxy_file0
        ...
        if RA_cen_init0 < RA_crit or RA_cen_init0 > 360.-RA_crit:
            RA_shift_flag = True
            RA_cen_init = RA_cen_init0 + 180.
            if RA_cen_init0 > 360:
                RA_cen_init = RA_cen_init0 - 360.         #Commentaire -> plutot RA_cen_init que RA_cen_init0 ?
            RA = RA + 180.
        ...
        if np.abs(Dec_cen_init) > Dec_crit:
            ...
            gal_l = coords_galactic.l.deg              #Commentaire -> gal_l gal_b peuvent etre directement remplacees
            gal_b = coords_galactic.b.deg
            RA_cen_init  = gal_l
            Dec_cen_init = gal_b
            coords = SkyCoord(RA,Dec,frame='icrs',unit='deg')
            coords_galactic = coords.galactic
            gal_l = coords_galactic.l.deg
            gal_b = coords_galactic.b.deg
            RA  = gal_l
            Dec = gal_b
            if verbosity >= 3:
        ...
        if np.max(cosR_sky > 1):                                                 #Commentaire -> parentheses etranges
        ...
        if verbosity >= 1:
            print("** MAIN: R_min R_max (deg) N_tot = ", R_min, R_max, len(R_sky[condition]))
        ...
        if verbosity >= 1:
            print("MAIN: ellipticity_pred=",ellipticity_pred," PA_pred =", PA_pred)

        print (" date/time          cluID/file              method      tol c e b int model    RA0       Dec0    log_r_{-2} ellip  PA log_bg   -lnL       BIC      Npasses")
                ...
                for background_flag in background_flags:
                    ...
                    for model in models:
                        ...
                        if not recenter_flag:
                            ...
                            RA_cen_minallow  = RA_cen_init  - 0.5 * R_max / np.cos(Dec_cen * degree)
                            RA_cen_maxallow  = RA_cen_init  + 0.5 * R_max / np.cos(Dec_cen * degree)
                        ...
                        if RA_shift_flag:
                            RA_cen_bestfit = RA_cen_bestfit - 180.
                            if verbosity >= 3:
                                print("shifting back RA, now at ", RA_cen_bestfit)
                        if RA_cen_bestfit < 0:
                            RA_cen_bestfit = RA_cen_bestfit + 360.

                        # convert central poistion back to equatorial coordinates if galactic coordinates had been used
                        if eq2gal_flag:
                            coords_galactic = SkyCoord(RA_cen,Dec_cen,frame='galactic',unit='deg')
                            coords_equatorial = coords_galactic.icrs
                            RA_cen  = coords_equatorial.ra
                            Dec_cen = coords_equatorial.dec
                        ...
                        print("arg=",maxR/10**log_scale_radius_bestfit)
                        print("model=",model)
        ...
        print        (fmt.format(date_time,version,galaxy_file,"median-sep",0,0,0,0,'x',"none",RA_cen_init0,Dec_cen_init0,log_median_separation,0,0,log_background_best_fit,0,0,0,0))
        f_out.write (fmt2.format(date_time,version,galaxy_file,"median-sep",0,0,0,0,'x',"none",RA_cen_init0,Dec_cen_init0,log_median_separation,0,0,log_background_best_fit,0,0,0,0))
