...
def convertModel(model):
    ...
    if model in ('n', 'N', 'nfw'):
        ...
        model2 = 'cNFW'
    elif model in ('u', 'U', 'uniform', 'UNIFORM'):
        model2 = 'Uniform'
    elif model in ('nt', 'NFWt', 'truncNFW', 'tNFW'):
        model2 = 'NFWtrunc'
    else:
        model2 = model
    return(model2)
...
def print_help():
    ...
    print("             Scale option. Change precision for erach argument.")
    print("    -e")
...
def Ask_input_values(params):
    ...
    if auto in ("manual", "auto"):
        ...
        try:
            ...
            with f_test:
                ...
                [verbosity, test_flag, model, cluster_richness, cluster_ellipticity, cluster_PA, background, cluster_loga, cluster_cen_RA, cluster_cen_Dec, Rmaxovervir, do_center, do_ellipticity,
                 N_points, do_background, method, tolerance, id_cluster_min_max, min_members, ran_list_asstr, data_galaxies_dir, median_flag, rescale_flag, output_file, detection_file, association_file] = params_list
    else:
        [verbosity, test_flag, model, cluster_richness, cluster_ellipticity, cluster_PA, background, cluster_loga, cluster_cen_RA, cluster_cen_Dec, Rmaxovervir, do_center, do_ellipticity,
                 N_points, do_background, method, tolerance, id_cluster_min_max, min_members, ran_list_asstr, data_galaxies_dir, median_flag, rescale_flag, output_file, detection_file, association_file] = params
    ...
    my_list = [verbosity, test_flag, model, cluster_richness, cluster_ellipticity, cluster_PA, background, cluster_loga, cluster_cen_RA, cluster_cen_Dec, Rmaxovervir, do_center, do_ellipticity,
            N_points,do_background, method, tolerance, id_cluster_min, id_cluster_max, min_members, rank_list_asstr, data_galaxies_dir, median_flag, output_file, detection_file, association_file]
    flags   = (recenter_flags, ellipticity_flags, background_flags, rescale_flag)
    for item in my_list:
        if verbosity >= 3:
            print("item = ", item)
        f_test.write("%s\n" % item)
...
if __name__ == '__main__':
    ...
    # author: Gary Mamon
    optlist, args = getopt.getopt(sys.argv[2:], "ecbaxv:m:M:r:t:d:o:n:s:l:D:s:", ['verb=', 'ver=', 'verbosity=', 'model=', 'mod=', 'meth=', 'method=', 'met=', 'datatype=', 'ofile=', 'minmax=', 'sub=', 'load=',
    ...
    verbosity, model, method, Rmaxovervir, tolerance, min_members, do_ellipticity  = 0, "NFW", "tnc", -1., 0.0001, 1, "n"
    ...
    n_subclusters, rescale_flag, extended_data_flag                                         = 1, "n", False
	...
	#Get/ask user/save input values from Ask_input_values function
    params_in = (verbosity, test_flag, model, cluster_richness, cluster_ellipticity, cluster_PA, background, cluster_loga, cluster_cen_RA, cluster_cen_Dec, Rmaxovervir, do_center, do_ellipticity,
                 N_points,do_background, method, tolerance, id_cluster_min_max, min_members, rank_list_asstr, data_galaxies_dir, median_flag, rescale_flag, output_file, detection_file, association_file)

    params_out, flags = Ask_input_values(params_in)
    (verbosity, test_flag, model, cluster_richness, cluster_ellipticity, cluster_PA, background, cluster_loga, cluster_cen_RA, cluster_cen_Dec, Rmaxovervir, do_center, do_ellipticity,
     N_points,do_background, method, tolerance, id_cluster_min, id_cluster_max, min_members, rank_list_asstr, data_galaxies_dir, median_flag, output_file, detection_file, association_file) = params_out
    (recenter_flags, ellipticity_flags, background_flags, rescale_flag) = flags

    #with fits.open("Part-1_mice2_500_cat_radec_final.fits", memmap=False) as hdu:
        #dat     = hdu[1].data['Z']

        #print(len(dat))
        #print(len(dat['ra_gal']))
        #exit()
    #########################################################
            ...
            for ellipticity_flag in ellipticity_flags:
                ...
                for background_flag in background_flags:
                    ...
                    if median_flag != "o":
                        ...
                        if extended_data_flag:
                            extended_data_file.write(str(id_cluster) + " " + method + " " + str(tolerance) + " " + str(recenter_flag) + " " + str(ellipticity_flag) + " " + str(background_flag)+ " " + str(sN_points) + " " + model + " " + str(round(RA_cen_pred, 1)) + " " + str(round(RA_cen_bestfit, 2)) + " " + str(round(Dec_cen_pred, 2)) + " " + str(round(Dec_cen_bestfit, 2)) + " " + str(round(log_scale_radius_pred, 4)) + " " + str(round(log_scale_radius_bestfit, 4)) + " " + str(round(true_radius, 4)) + " " + str(round(ellipticity_pred, 3)) + " " + str(round(ellipticity_bestfit, 3)) + " " + str(int(PA_pred)) + " " + str(int(PA_bestfit)) + " " + str(round(log_background_pred, 4)) + " " + str(round(log_background_bestfit, 4)) + " " + str(fit_results.fun) + " " + str(time_taken) + "\n")
                        else:
