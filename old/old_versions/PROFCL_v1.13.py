...
* Version 1.13 (3 May 2018)
        ...
        > Monte Carlo code corrected (cluster shape is now correct)
        > Probability computation with Monte Carlo corrected (projected number between R_min and R_max now positive definite)
...
if __name__ == '__main__':
    ...
    #                                                                                                                                                           #
    # Typical command line : python PROFCL_v1.13.py -man -verb 0 -model NFW -meth tnc -tol 0.0001 -ellip 0 -median -center -minmax 0 99 -mock A 1280 0.5 50 20  #
    #                                                                                                                                                           #
    ...
    #############################################################################################################################################################
    optlist, args = getopt.getopt(sys.argv[1:], "ecbDv:m:M:r:t:d:o:n:", ['verb=', 'ver=', 'verbosity=', 'model=', 'mod=', 'meth=', 'method=', 'met=', 'datatype=', 'ofile=', 'minMax=',
                                 'RmaxoverVir=', 'Rmaxovervir=', 'rmaxoverVir=', 'rmaxovervir=', 'tol=', 'tolerance=', 'minmemb=', 'Minmemb=', 'mm=', 'dtype=', 'outputfile=', 'MinMax=',
                                 'minmem=', 'Minmem=', 'datadir=', 'ddir=', 'datadirectory=', 'Npoints=', 'npoints=', 'output=', 'median=', 'med=', 'minmax=', 'tole=', 'Minmax='])
    ...
    size = len(sys.argv[1:])
    if size >=1 and sys.argv[1] == "-h":
        ### ECRIRE NOTICE D'AIDE ICI ###
        ...
        for opt, arg in optlist:
            if opt in ("-v", "--ver", "--verb", "--verbosity"):
                ...
                min_members = int(arg)
            elif opt in ("--minmax", "--Minmax", "--MinMax", "--minMax", "-D", "--datatype", "--dtype"):
                for i in range(1, len(sys.argv)):
                    if sys.argv[i] in ("--minmax", "--Minmax", "--MinMax", "--minMax"):
                        id_cluster_min_max = sys.argv[i+1] + " " + sys.argv[i+2]
                    elif sys.argv[i] in ("-D", "--datatype", "--dtype"):
                        test_flag = sys.argv[i+1]
                        if test_flag == "A": #Emmanuel Academic must be followed in the same order by cluster richness (number), ellipticity (number), PA (number), background (number)
                            cluster_richness    = int(sys.argv[i+2])
                            cluster_ellipticity = float(sys.argv[i+3])
                            cluster_PA          = float(sys.argv[i+4])
                            background          = int(sys.argv[i+5])
            else:
    ...
    #print(verbosity, model_test_tmp, method, Rmaxovervir, tolerance, min_members)
    #print( data_galaxies_dir, do_ellipticity, test_flag, median_flag, do_center, do_background, id_cluster_min_max, output_file)
    #print(cluster_richness, cluster_ellipticity, cluster_PA, background)
    ...
    if auto == "manual" or auto == "auto":
        ...
        try:
            ...
            with f_test:
                ...
                [verbosity,test_flag,model_test_tmp,cluster_richness,cluster_ellipticity,cluster_PA,background,Rmaxoverrvir,do_center,do_ellipticity,N_points,do_background,method,tolerance,id_cluster_min_max,min_members,data_galaxies_dir, median_flag, output_file] = params_list
