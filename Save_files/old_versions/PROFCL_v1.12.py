...
def Random_radius(Xmax,model,Npoints,Nknots):
    ...
    #lasinhX0 = np.arcsinh(np.linspace(0, Xmax, num=Nknots+1))
...
def Random_xy(Rmax,model,Npoints,Nknots,ellipticity,PA):
    ...
    #print("IS TRUE : ", x0_random == RA_cen_init)
...
if __name__ == '__main__':
    ...
    if nb_arguments >= 2:
        ...
        if sys.argv[1] == "-man":
            #default values
            verb, model_test_tmp, method, Rmaxovervir, tolerance, min_members, data_galaxies_dir, do_ellipticity = 0, "NFW", "tnc", -1., 0.0001, 1, "galaxies/", "n"
            median_flag, id_cluster_min_max, do_center, do_background                                            = "y", [0, 1], "n", "n"

            auto = "options"
            list_arg = sys.argv[2:]
            for i in range(len(list_arg)):
                if list_arg[i] == "-verb": #verbosity must be followed by a number between 0 and 4
                    i += 1
                    verbosity = int(list_arg[i])
                if list_arg[i] == "-model": #model must be followed by a string
                    i += 1
                    model_test_tmp = list_arg[i]
                if list_arg[i] == "-meth": #method must be followed by a string
                    i += 1
                    method = list_arg[i]
                if list_arg[i] == "-RmaxoverVir": #ratio of radii must be followed by a number
                    i += 1
                    Rmaxovervir = float(list_arg[i])
                if list_arg[i] == "-tol": #tolerance must be followed by a number
                    i += 1
                    tolerance = float(list_arg[i])
                if list_arg[i] == "-minmemb": #minimum member must be followed by a number
                    i += 1
                    min_members = int(list_arg[i])
                if list_arg[i] = "-datadir": #directory of galaxy data must be followed by a string
                    i += 1
                    data_galaxies_dir = list_arg[i]
                if list_arg[i] == "-ellip": #ellipticity must be followed by a number
                    i += 1
                    N_points       = int(list_arg[i])
                    do_ellipticity = "e"

                if list_arg[i] = "-median": #median separation flag must either be followed by "only" for computing only median separation or by nothing
                    if list_arg[i+1] == "only":
                        i += 1
                        median_flag = list_arg[i]
                    else:
                        median_flag = "y"

                if list_arg[i] == "-minmax": #minimum and maximum ID clusters must be followed by two numbers (like this for instance : 0 99)
                    id_cluster_min_max[0] = int(list_arg[i+1])
                    id_cluster_min_max[1] = int(list_arg[i+2])
                    i += 2

                if list_arg[i] == "-center": #centering option must be followed by nothing (other than another option)
                    do_center = "c"
                if list_arg[i] == "-back": #background option
                    do_background = "bg"

                if list_arg[i] == "-mock": #mock choice
                    i += 1
                    mock_type = list_arg[i]
                    if mock_type == "A": #Emmanuel Academic must be followed in the same order by cluster richness (number), ellipticity (number), PA (number), background (number)
                        cluster_richness    = int(list_arg[i+1])
                        cluster_ellipticity = float(list_arg[i+2])
                        cluster_PA          = float(list_arg[i+3])
                        background          = int(background)
                        i += 4

                else:
                    print("Unknow parameter ", list_arg[i])
                    exit(-1)
    ...
    try:
        ...
        with f_test:
            ...
            [verbosity,test_flag,model_test_tmp,cluster_richness,cluster_ellipticity,cluster_PA,background,Rmaxoverrvir,do_center,do_ellipticity,N_points,do_background,method,tolerance,id_cluster_min_max,min_members,data_galaxies_dir, median_flag, auto] = params_list
    ...
    my_list = [verbosity,test_flag,model_test_tmp,cluster_richness,cluster_ellipticity,cluster_PA,background,Rmaxoverrvir,do_center,do_ellipticity,N_points,do_background,method,tolerance,id_cluster_min_max,min_members, data_galaxies_dir, median_flag, auto]
