...
def PROFCL_prob_uv(u, v, R_min, R_max, scale_radius, ellipticity, background, R_cut, model):
    ...
    #print(SurfaceDensity_tilde(R_circ/a, R_cut/a, model), Nofa, scale_radius)
...
def PROFCL_LogLikelihood(params, data, R_min, R_max, model):
    ...
    if model == "NFWtrunc" and R_cut > log_scale_radius:
        tmp = log_scale_radius
        log_scale_radius = R_cut
        R_cut = tmp
    ...
    for i in range(len(params)):
        sumPenalization += PenaltyFunction(params[i],bounds[i,0],bounds[i,1])
...
if __name__ == '__main__':
    ...
    if extended_data_flag:
        ...
        extended_data_file.write("cluster_id method model_cluster tol clust_lsr_pred clust_lsr_bestfit halo_lsr_bestfit med_sep_lsr -ln(likelihood_clust) -ln(likelihood_halo) N_halo M_AMICO(sum of p) M_halo SNR Size(Mpc/h) time_taken\n")
    ...
    for id_cluster in cluster_range:
        ...
        #select galaxies coordinates and probabilities of membership in current cluster
            ...
            h_ngals         = halo_NGALS[ID == id_cluster][0]
            RA_H            = RA_gal_ofH[Full_halo_ID == id_halo]
            Dec_H           = Dec_gal_ofH[Full_halo_ID == id_halo]

        #ftest = open("Halo1.dat", "w")
        #for i in range(len(Dec_H)):
        #    print(RA_H[i])
        #    ftest.write(str(RA_H[i]) + " " + str(Dec_H[i]) + "\n")
        ...
        if test_flag == "d":
            RA_cen_initH, Dec_cen_initH  = guess_center(RA_H, Dec_H)
        ...
        if test_flag == "d":
            if RA_cen_initH < RA_crit or RA_cen_initH > 360.-RA_crit:
                RA_shift_flagH  = True
                RA_cen_initH    += 180.
                if RA_cen_initH > 360:
                    RA_cen_initH -= 360.
                RA_H += 180.
                RA_H = np.where(RA_H > 360., RA_H-360., RA_H)
            else:
                RA_shift_flagH = False
        ...
        if test_flag == "d":
            # convert to galactic if near pole
            if np.abs(Dec_cen_initH) > Dec_crit:
                eq2gal_flagH = True
                coords_cenH = SkyCoord(RA_cen_initH, Dec_cen_initH, frame='icrs', unit='deg')
                coords_galacticH = coords_cenH.galactic
                RA_cen_initH    = coords_galacticH.l.deg
                Dec_cen_initH   = coords_galacticH.b.deg

                coordsH = SkyCoord(RA_H, Dec_H, frame='icrs', unit='deg')
                coords_galacticH = coordsH.galactic
                RA_H    = coords_galacticH.l.deg
                Dec_H   = coords_galacticH.b.deg
            else:
                eq2gal_flagH = False
        ...
        R_sky   = np.sqrt( ((RA-RA_cen_init)*np.cos(Dec_cen_init*degree))**2 + (Dec-Dec_cen_init)**2 )
        maxR    = np.max(R_sky)
        print("maxR:", maxR)
        R_min   = R_min_over_maxR * maxR

        if test_flag == "d":
            R_skyH   = np.sqrt( ((RA_H - RA_cen_initH)*np.cos(Dec_cen_initH*degree))**2 + (Dec_H - Dec_cen_initH)**2 )
            maxRH    = np.max(R_skyH)
            R_minH   = R_min_over_maxR * maxRH
            print("maxRH":, maxRH)
        ...
        if test_flag == "d":
            conditionH = np.logical_and(R_skyH >= R_minH, R_skyH <= R_maxH)
        ...
        if test_flag == "d":
            log_background_maxallow_defaultH = np.log10(len(R_skyH[conditionH])/(np.pi*(R_maxH*R_maxH-R_minH*R_minH)))
        ...
        if test_flag == "d":
            RA_H    = RA_H[conditionH]
            Dec_H   = Dec_H[conditionH]
            prob_membershipH = 0*RA_H+1
        ...
        # Compute physical size of cluster
        def eta(a, O):
            s3  = (1-O)/O
            s   = s3**(1/3)
        ...
        if test_flag == "d":
            ellipticity_predH, PA_predH = guess_ellip_PA(RA_H, Dec_H, RA_cen_initH, Dec_cen_initH)
        ...
        for recenter_flag in recenter_flags:
            ...
            for ellipticity_flag in ellipticity_flags:
                ...
                for background_flag in background_flags:
                    ...
                    log_scale_radius = np.log10 ( 1.35 * np.median(R_sky) )   *0 # Factor 1.35 is predicted median for NFW and coredNFW of concentration 3

                    if test_flag == "d":
                        log_scale_radius_maxallowH = np.log10(R_minH / min_R_over_rminus2)
                        if R_minH == 0:
                            log_scale_radius_minallowH = log_scale_radius_maxallowH - 2
                        else:
                            log_scale_radius_minallowH = np.log10(R_maxH / max_R_over_rminus2)
                            ### FUDGE FOR TESTING
                            log_scale_radius_minallowH = np.log10(R_minH)
                            log_scale_radius_maxallowH = np.log10(R_maxH)

                        log_scale_radiusH = np.log10 ( 1.35 * np.median(R_skyH) )   *0 # Factor 1.35 is predicted median for NFW and coredNFW of concentration 3
                    ...
                    if test_flag == "d":
                        RA_cen_minallowH, RA_cen_maxallowH, Dec_cen_minallowH, Dec_cen_maxallowH = [RA_cen_initH]*2 + [Dec_cen_initH]*2
                        RA_cenH, Dec_cenH = RA_cen_initH, Dec_cen_initH
                        ellipticity_minallowH, ellipticity_maxallowH, PA_minallowH, PA_maxallowH, ellipticityH, PAH = [0]*6
                        log_background_minallowH, log_background_maxallowH, log_backgroundH = [-99]*3
                        R_cut_minallowH, R_cut_maxallowH, R_cutH = R_maxH / 5, 2 * R_maxH, 0.7 * R_maxH
                    ...
                    if median_flag != "o":
                        ...
                        if test_flag == "d":
                            dataH   = [RA_H, Dec_H, prob_membershipH]
                        ...
                        if test_flag == "d":
                            if rescale_flag == "y":
                                paramsH  = [RA_cenH/(R_maxH*np.cos(Dec_cenH*degree)), Dec_cenH/R_maxH, log_scale_radiusH, ellipticityH/5., PAH/500., log_backgroundH/10., np.log10(R_cutH)/5.]
                                boundsH = np.array([(RA_cen_minallowH/(R_maxH*np.cos(Dec_cenH*degree)), RA_cen_maxallowH/(R_maxH*np.cos(Dec_cenH*degree))),
                                                   (Dec_cen_minallowH/R_maxH, Dec_cen_maxallowH/R_maxH),
                                                   (log_scale_radius_minallowH, log_scale_radius_maxallowH),
                                                   (ellipticity_minallowH/5., ellipticity_maxallowH/5.),
                                                   (PA_minallowH/500., PA_maxallowH/500.),
                                                   (log_background_minallowH/10., log_background_maxallowH/10.),
                                                   (np.log10(R_cut_minallowH)/5., np.log10(R_cut_maxallowH)/5.)])
                            else:
                                paramsH  = [RA_cenH, Dec_cenH, log_scale_radiusH, ellipticityH, PAH, log_backgroundH, np.log10(R_cutH)]
                                boundsH = np.array([(RA_cen_minallowH, RA_cen_maxallowH),
                                                   (Dec_cen_minallowH, Dec_cen_maxallowH),
                                                   (log_scale_radius_minallowH, log_scale_radius_maxallowH),
                                                   (ellipticity_minallowH, ellipticity_maxallowH),
                                                   (PA_minallowH, PA_maxallowH),
                                                   (log_background_minallowH, log_background_maxallowH),
                                                   (np.log10(R_cut_minallowH), np.log10(R_cut_maxallowH))])
                        print("boundsh", boundsH)
                        ...
                        if test_flag == "d":
                            fit_resultsH = PROFCL_Fit(dataH, paramsH,
                                                    boundsH,
                                                    R_minH, R_maxH, "NFW",
                                                    False, False, False,
                                                    PROFCL_LogLikelihood, "TNC")
                        ...
                        print(fit_results)
                        if test_flag == "d":
                            params_bestfitH = fit_resultsH.x
                            print(fit_resultsH)
                        ...
                        if test_flag == "d":
                            [RA_cen_bestfitH, Dec_cen_bestfitH, log_scale_radius_bestfitH, ellipticity_bestfitH, PA_bestfitH, log_background_bestfitH, R_cut_bestfitH] = params_bestfitH
                            if rescale_flag == "y":
                                Dec_cen_bestfitH         *= R_maxH
                                RA_cen_bestfitH          *= R_maxH*np.cos(Dec_cen_bestfitH*degree)
                                ellipticity_bestfitH     *= 5.
                                PA_bestfitH              *= 500.
                                log_background_bestfitH  *= 10.
                                R_cut_bestfitH           = 10**(5.*R_cutH)
                        ...
                        if test_flag == "d":
                            if eq2gal_flagH:
                                coords_galacticH = SkyCoord(RA_cenH, Dec_cenH, frame='galactic', unit='deg')
                                coords_equatorialH = coords_galacticH.icrs
                                RA_cen_predH  = coords_equatorialH.ra
                                Dec_cen_predH = coords_equatorialH.dec

                        # shift back RA_cen if initial RA_cen was close to 0 and make sure that RA_cen is not negative
                        if test_flag == "d":
                            if RA_shift_flagH:
                                RA_cen_bestfitH -= 180.
                            if RA_cen_bestfitH < 0:
                                RA_cen_bestfitH += 360.
                        ...
                        if test_flag == "d":
                            BICH    = 2.*fit_resultsH.fun + 2 * np.log(N_dataH)
                        ...
                        if test_flag == "d":
                            numH = Ntot_maxRH - np.pi * maxRH**2 * 10**log_background_bestfitH
                            denomH = Number_tilde(maxRH/10**log_scale_radius_bestfitH, model)
                            N_r_minus_bestfitH = numH/denomH

                        # print results to file
                        if extended_data_flag:
                            extended_data_file.write(str(id_cluster) + " " + method + " " + model + " " + str(round(tolerance,5)) + str(round(log_scale_radius_pred,3)) + " " + 
                                                     str(round(log_scale_radius_bestfit,3)) + " " + str(round(log_scale_radius_bestfitH,3)) + " " + str(round(log_median_separation,3)) + " " + 
                                                     str(round(fit_results.fun,2)) + " " + str(round(fit_resultsH.fun,2)) + " " +  str(int(h_ngals)) + " " + str(round(mass_AMICO,3)) + " " + 
                                                     str(round(mass,3)) + " " + str(round(SNR_clust,3)) + " " + str(round(size,2)) + " " + str(round(time_taken,3))  + "\n")
