...
def convertMethod(method):
    ...
    if np.any(np.array(['NM','nm','nelder-mead']) == method):
        method2 = 'Nelder-Mead'
    elif np.any(np.array(['powell','pow','Pow']) == method):
        method2 = 'Powell'
    elif method == 'cg':
        method2 = 'CG'
    elif method == 'bfgs':
        method2 = 'BFGS'
    elif np.any(np.array(['newton-cg','ncg','NCG']) == method):
        method2 = 'Newton-CG'
    elif np.any(np.array(['l-bfgs-b','LBB','lbb']) == method):
        method2 = 'L-BFGS-B'
    elif method == 'slsqp':
        method2 = 'SLSQP'
    elif np.any(np.array(['tnc','t']) == method):
        method2 = 'TNC'
    elif method == 'fmin_TNC':
        method2 = 'fmin_tnc'
    elif np.any(np.array(['de','diffevol','DiffEvol']) == method):
        method2 = 'Diff-Evol'
    elif method == 'T':
        method2 = 'test'
    else:
...
def convertModel(model):
    ...
    if np.any(np.array(['n','N','nfw','Nfw']) == model):
        model2 = 'NFW'
    elif np.any(np.array(['c','C','cnfw','coredNFW']) == model):
        model2 = 'cNFW'
    elif np.any(np.array(['u','U','uniform']) == model):
        model2 = 'Uniform'
    elif model == 'nt' or model == 'NFWt':
        model2 = 'NFWtrunc'
    else:
        model2 = model
    return(model2)
...
if __name__ == '__main__':
    ...
    if auto == "manual":
        ...
        if 'test_flag' in locals():
            test_flag = input("Enter d for data, a for academic (Gary), A for academic (Emmanuel), f for Flagship, m for other mock [%s]: "%test_flag) or test_flag
        else:
            test_flag = input("Enter d for data, a for academic (Gary), A for academic (Emmanuel), f for Flagship, m for other mock: ")
    verbosity = int(verbosity)

    # questions for cluster file
    if np.any(np.array(['d','f']) == test_flag):
        cluster_richness = -1
    ...
    elif np.any(np.array(['a','A','b']) == test_flag):
        if auto == "manual":
    ...
    if auto == "manual":
        ...
        while ok == 0:
            ...
            if np.any(np.array(['c','n','b']) == do_center):
                ok = 1
    if do_center == 'c':
    ...
    if auto == "manual":
        ...
        while ok == 0:
            ...
            if np.any(np.array(['e','n','b']) == do_ellipticity):
                ok = 1
    if do_ellipticity == 'e':
    ...
    if auto == "manual":
        ...
        while ok == 0:
            ...
            if np.any(np.array(['bg','n','b']) == do_background):
                ok = 1
    if do_background == 'bg':
